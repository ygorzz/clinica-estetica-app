  ...
  <div class="dashboard-container">
    ...
    <div v-if="showEditModal" class="modal-overlay" @click.self="fecharModalEdicao">
      <div class="modal-content">
        ...
        <form @submit.prevent="updateAgendamento" class="modal-form">
          <div class="form-group"> <label>Data</label> <input v-model="editingAgendamento.data" type="date" required class="input-field" /> </div>
          <div class="form-group"> <label>Hora</label> <input v-model="editingAgendamento.hora" type="time" required class="input-field" /> </div>
          <div class="form-group"> <label>Serviço</label> <select v-model="editingAgendamento.servicoId" required class="input-field"> <option v-for="servico in servicos" :key="servico.id" :value="servico.id">{{ servico.nome }}</option> </select> </div>
          <div class="form-group"> <label>Cliente</label> <select v-model="editingAgendamento.clienteId" required class="input-field"> <option v-for="cliente in clientes" :key="cliente.id" :value="cliente.id">{{ cliente.nome }}</option> </select> </div>
          <div class="form-group"> <label>Funcionário</label> <select v-model="editingAgendamento.userId" required class="input-field"> <option v-for="usuario in usuarios" :key="usuario.id" :value="usuario.id">{{ usuario.fullName }}</option> </select> </div>
          <div class="modal-actions"> <button @click="fecharModalEdicao" type="button" class="button cancel-button">Cancelar</button> <button type="submit" class="button">Salvar Alterações</button> </div>
        </form>
      </div>
    </div>
    ...
    <div v-if="showConfirmDeleteModal" class="modal-overlay" @click.self="fecharModalDelecao">
      <div class="modal-content">
        ...
        <div class="modal-actions"> <button @click="fecharModalDelecao" class="button cancel-button">Cancelar</button> <button @click="confirmarDelecao" class="button delete-button-modal">Confirmar Exclusão</button> </div>
      </div>
    </div>
    ...
    <div v-if="showFilterModal" class="modal-overlay" @click.self="fecharModalFiltro">
      <div class="modal-content">
        ...
        <div class="filter-grid modal-form"> 
          <div class="form-group"> <label>Data</label> <input type="date" v-model="filtros.data" class="input-field filter-field" /> </div>
          <div class="form-group"> <label>Hora</label> <input type="time" v-model="filtros.hora" class="input-field filter-field" /> </div>
          <div class="form-group"> <label>Serviço</label> <select v-model="filtros.servicoId" class="input-field filter-field"> <option :value="null">Todos Serviços</option> <option v-for="servico in servicos" :key="servico.id" :value="servico.id">{{ servico.nome }}</option> </select> </div>
          <div class="form-group"> <label>Cliente</label> <select v-model="filtros.clienteId" class="input-field filter-field"> <option :value="null">Todos Clientes</option> <option v-for="cliente in clientes" :key="cliente.id" :value="cliente.id">{{ cliente.nome }}</option> </select> </div>
          <div class="form-group"> <label>Funcionário</label> <select v-model="filtros.userId" class="input-field filter-field"> <option :value="null">Todos Funcionários</option> <option v-for="usuario in usuarios" :key="usuario.id" :value="usuario.id">{{ usuario.fullName }}</option> </select> </div>
        </div>
        <div class="modal-actions"> <button @click="limparFiltrosEClose" class="button cancel-button">Limpar</button> <button @click="aplicarFiltrosEClose" class="button">Aplicar Filtros</button> <button @click="fecharModalFiltro" type="button" class="button cancel-button">Cancelar</button> </div>
      </div>
    </div>
  </div>
</template>
...
.list-header {
  ...
  gap: 1rem; 
  margin-bottom: 1.5rem; /* Espaçamento antes da tabela/mensagem */
}
...
/* Ajuste para o título do formulário de adição */
.section-card > .section-title:not(:first-child) { 
...
@media (max-width: 768px) {
  .title-toggle-wrapper {
     ...
     /* Não precisa de margem inferior, o gap do list-header cuida disso */
  }
   .filter-icon-button {
